        ...
        if (query != null && !query.isBlank()) {
        	...
        	sql.append("""
                    AND (
                        ...
                        OR LOWER(cl.name) LIKE LOWER(?) 
                        OR LOWER(s.name) LIKE LOWER(?) 
                        OR LOWER(b.code) LIKE LOWER(?) 
                        OR LOWER(a.complete_text) LIKE LOWER(?)
                        OR a.latitude LIKE ?
                        OR a.longitude LIKE ?
                    )
                """);
            
            // add params for all fields
            for (int i = 0; i < 10; i++) {
                params.add("%" + query + "%");
            }
        }
...
//        } 
        ...
        sql.append(" ORDER BY a.complete_text ASC");
        return jdbcTemplate.query(sql.toString(), AddressRepository::mapRowToAddressDTO, params.toArray());
    ...
    public AddressDTO lookupAddress(BigDecimal lat, BigDecimal lng) {
        ...
        String sql = """
            SELECT 
                ...
                s.name AS section_name, 
                rd.name AS road_name,
                SQRT(POW(a.latitude - ?, 2) + POW(a.longitude - ?, 2)) AS distance
            FROM addresses a
            LEFT JOIN countries c ON a.country_id = c.id
            LEFT JOIN regions r ON a.region_id = r.id
            LEFT JOIN prefectures p ON a.prefecture_id = p.id
        ...
        return jdbcTemplate.queryForObject(sql, AddressRepository::mapRowToAddressDTO, lat, lng);
    }
